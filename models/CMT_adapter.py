...
class CMT_adapter(nn.Module):
    ...
    def __init__(self,
        ...
        self.proj_text_down = nn.Sequential(nn.Linear(in_channels_txt, adapter_channels, bias=False))
    ...
    def forward(self, vis, clip_length, text, view_tokens=None):
        ...
        if self.use_HSA:
            ...
            x_hsa = einops.rearrange(
                x_hsa,
                'b t (h1 h) (w1 w) c -> b t (h1 w1) (h w) c',
                h=self.HSA_patch_size,
                w=self.HSA_patch_size,
            )
            x_hsa = x_hsa + self.time_embed(x_hsa).permute((0, 2, 1))[:, :, None, None, :] + self.spatial_pos_embed
            x_hsa = einops.rearrange(x_hsa, 'b t N p c -> (t p) (b N) c')
            ...
            x_hsa = einops.rearrange(
                ...
                '(t p) (b N) c -> b t N p c',
                t=int(clip_length),
                b=int(BT / clip_length),
                p=self.HSA_patch_size ** 2,
            )
            x_hsa = einops.rearrange(
                x_hsa,
                'b t (h1 w1) (h w) c -> (h1 h w1 w) ( b t) c',
                h1=int(H / self.HSA_patch_size),
                w1=int(W / self.HSA_patch_size),
                h=self.HSA_patch_size,
                w=self.HSA_patch_size,
            )
        ...
        # t includes the appended view token when provided
        x_atten = self.ca_V2T(x_atten, memory=t.repeat_interleave(clip_length, 1))
        ...
        t_atten = self.ca_T2V(t, v_memory)
        ...
        return x_out, t_out, view_token_out_b
