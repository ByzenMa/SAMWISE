...
def collate_fn(batch):
    ...
    batch[0] = nested_tensor_from_videos_list(batch[0])
...
def targets_to(targets: List[Dict[str, Any]], device):
    ...
    if "dataset_name" in targets[0]:
        ...
        return [{k: v.to(device)  for k, v in t.items() if k not in ["caption", "dataset_name", "original_id", "image_id", "id_tuple"]} for t in targets]
...
def on_load_checkpoint(model, checkpoint) -> None:
